    ...
    return function HOC() {
        ...
        return (
            <div className="relative">
                {
                    showPagination && <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-gray-800 shadow-lg z-10 flex text-sm flex-col sm:flex-row justify-center items-center gap-4 py-2">
                        ...
                        <span>
                            <select className="rounded-full text-slate-900 border-slate-900 border px-2" value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))} name="" id="">
                                {
                                    [5, 10, 15, 20, 25, 30, 35, 50].map((pageSizeClient, index) => {
                                        return <option value={pageSizeClient} key={index}>Show - {pageSizeClient}</option>
                                    })
                                }
                            </select>
                        </span>
                ...
                <div className={`${containerClassname}`}>
                    <div className="text-xl text-center underline font-bold mt-8">{heading}</div>
                                ...
                                <tr {...headerGroup.getHeaderGroupProps()} key={index}>
                                    {headerGroup.headers.map((column, index) => (
                                        ...
                                        >
                                            ...
                                            {(column.render('Header') !== 'Action') && <div className="text-xs font-thin">{column.isSorted ? column.isSortedDesc ? <div className="flex justify-start items-center">Desc <FaSortUp /></div> : <div className="flex justify-start items-center">Asc <FaSortDown /></div> : <div className="flex justify-start items-center">Sort <TiArrowUnsorted /></div>}</div>}
                                        </th>
                                    ))}
                                </tr>
                            ))}
